    ...
    if menu == "Business Overview":
        ...
        with col2:
            ...
            st.metric("Tingkat Dropout Saat Ini", f"{dropout_rate:.2f}%")
                    # --- TAMBAHAN: ROW METRICS (Seperti Gambar) ---
...
# 2. DATA DASHBOARD (EDA)
        ...
        # Row 1: Status Distribution & Scholarship
...
# 3. DROPOUT PREDICTOR
        ...
        with st.form("input_form"):
            ...
            with col_new1:
                jalur_map = {
                    "Fase 1 - Umum": 1, "Fase 2 - Umum": 2, "Usia > 23 Tahun": 5,
            ...
            with col_new2:
                prodi_map = {
                    "Keperawatan": 9500, "Manajemen": 9147, "Layanan Sosial": 9238,
            ...
            #  KOLOM 1: Fokus pada Identitas Kursus & Gender 
            with c1:
                course_id = prodi_map[prodi_pilihan]
                st.info(f"**ID Kursus:** {course_id}\n\n**Bidang:** {prodi_pilihan}")
                gender = st.selectbox("Jenis Kelamin", ["Perempuan", "Laki-laki"])
                age = st.number_input("Usia saat Pendaftaran", 17, 70, 20)
            
            with c2:
                marital = st.selectbox("Status Pernikahan", ["Single", "Married", "Widower", "Divorced", "Facto Union", "Legally Seperated"])
            ...
            with c3:
                ...
                attendance = st.selectbox("Waktu Kuliah", ["Siang", "Malam"])

            st.markdown("### Performa 2 Semester & Nilai Masuk")
            c4, c5, c6 = st.columns(3)
            with c4:
                u1_enrolled = st.number_input("Unit Sem 1 Terdaftar", 0, 30, 6)
                u1_approved = st.number_input("Unit Sem 1 Lulus", 0, 30, 6)
                # Diubah ke skala 4.00
                u1_grade_4 = st.number_input("Nilai Sem 1 (Skala 4.00)", 0.0, 4.0, 3.0, step=0.01)

            with c5:
                u2_enrolled = st.number_input("Unit Sem 2 Terdaftar", 0, 30, 6)
                u2_approved = st.number_input("Unit Sem 2 Lulus", 0, 30, 6)
                # Diubah ke skala 4.00
                u2_grade_4 = st.number_input("Nilai Sem 2 (Skala 4.00)", 0.0, 4.0, 3.0, step=0.01)

            with c6:
                # Nilai Masuk dan Kualifikasi biasanya skalanya lebih besar (misal 0-200), 
                # Jika ingin tetap 4.00, sesuaikan di sini:
                admission_grade_4 = st.number_input("Nilai Masuk (Skala 4.00)", 0.0, 4.0, 2.4, step=0.01)
                prev_grade_4 = st.number_input("Nilai Kualifikasi Sebelumnya (Skala 4.00)", 0.0, 4.0, 2.4, step=0.01)
                unemployment = st.number_input("Laju Pengangguran (%)", 0.0, 20.0, 7.0)
                submit = st.form_submit_button(" Jalankan Prediksi")

        if submit:
            # Mapping Input ke Numeric
            mapping_gender = {"Perempuan": 0, "Laki-laki": 1}
            mapping_bool = {"Tidak": 0, "Ya": 1}
            mapping_marital = {"Single": 1, "Married": 2, "Widower": 3, "Divorced": 4, "Facto Union": 5, "Legally Seperated": 6}
            mapping_attendance = {"Siang": 1, "Malam": 0}

            input_dict = {col: df_raw[col].median() for col in feature_cols}
            
            # Update dengan input dari user termasuk fitur baru
            input_dict.update({
                ...
                'Curricular_units_1st_sem_grade': u1_grade_4,      
                'Curricular_units_2nd_sem_enrolled': u2_enrolled,
                'Curricular_units_2nd_sem_approved': u2_approved,
                'Curricular_units_2nd_sem_grade': u2_grade_4,      
                'Admission_grade': admission_grade_4,             
                'Previous_qualification_grade': prev_grade_4,     
                'Unemployment_rate': unemployment,
                'Daytime_evening_attendance': mapping_attendance[attendance],
                'Displaced': mapping_bool[displaced]
            })
