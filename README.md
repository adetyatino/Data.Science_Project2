...
2. Selanjutnya pastikan sudah memiliki file-file berikut dalam satu folder proyek:
    ...
    - Folder models (isinya yaitu: best_model_churn.pkl, encode.pkl, scaler.pkl)
        submission/
        ├── model/
...
3. Jika baru pertama kali mendapatkan folder proyek (yang berisi app.py dan notebook.ipynb). Library yang harus dimiliki yaitu pandas, numpy, streamlit, joblib, dan plotly. Jika belum ada requirements.txt, jalankan:
```bash
pip install pandas streamlit joblib plotly numpy 
``` 
- pandas: Untuk manipulasi data (dataframes).
- streamlit: Untuk membuat antarmuka web aplikasi.
- joblib: Untuk memuat model machine learning yang sudah disimpan.
- plotly: Untuk membuat grafik interaktif (mencakup express dan graph_objects).
- numpy: Untuk operasi numerik dan array.
- datetime dan os adalah pustaka bawaan Python (standard library), sehingga pengguna tidak perlu menginstalnya secara terpisah.

4. Terkahir jalankan Streamlit app.py dengan mengetik perintah berikut di terminal:
pastikan folder sudah benar (sesuai langkah No. 2)
```bash
streamlit run app.py
```

5. Selanjutnya Install All the Requirements Inside "requirements.txt" dengan mengetik perintah berikut di terminal:
```bash
...
Instal semua pustaka (library) Python yang digunakan dalam aplikasi dengan menjalankan perintah berikut:

``` bash
pip install streamlit pandas joblib plotly numpy scikit-learn xgboost
```
(Catatan: Karena model menggunakan XGBoost berdasarkan isi notebook, pastikan library tersebut terinstal agar model bisa dimuat).

3. Menjalankan Aplikasi Streamlit
Gunakan perintah berikut di terminal untuk menjalankan server lokal aplikasi:

``` bash
streamlit run oke.py
```
4. Mengakses Aplikasi Setelah perintah dijalankan, sistem akan otomatis membuka browser default dan menampilkan antarmuka dashboard pada alamat: http://localhost:8501 Fitur Utama Prototype. Setelah aplikasi berjalan, akan melihat beberapa bagian utama:
